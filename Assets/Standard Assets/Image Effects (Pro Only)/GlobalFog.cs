...
public class GlobalFog : MonoBehaviour {
	...
	void OnRenderImage (RenderTexture source, RenderTexture destination ) {	
		...
		Vector3 topRight = new Vector3(GetComponent<Camera>().transform.forward * CAMERA_NEAR + toRight + toTop);
		topRight.Normalize();
		topRight *= CAMERA_SCALE;
		
		Vector3 bottomRight = new Vector3(GetComponent<Camera>().transform.forward * CAMERA_NEAR + toRight - toTop);
		bottomRight.Normalize();
		bottomRight *= CAMERA_SCALE;
		
		Vector3 bottomLeft = new Vector3(GetComponent<Camera>().transform.forward * CAMERA_NEAR - toRight - toTop);
