Shader "RenderFX/Skybox Blended Fog 1" {
Properties {
    _Tint ("Tint Color", Color) = (.5, .5, .5, .5)
    _Blend ("Sky Blend", Range(0.0,1.0)) = 0.5
    _FogTint ("Fog Tint", Color) = (.5, .5, .5, .5)
    _BlendHorizon ("Horizon Blend", Range(0.0,1.0)) = 0.5
    ...
    _FogSideText("Side FogMask", 2D) = "white" {}
    _FogBottomText("Bottom FogMask", 2D) = "white" {}
}
...
SubShader {
    ...
    Pass {
        ...
        SetTexture [_FogSideText] { constantColor[_FogTint] combine constant lerp (texture) previous }
    }
    Pass {
    ...
    Pass {
        ...
        SetTexture [_FogBottomText] { constantColor[_FogTint] combine constant lerp (texture) previous }
    }
}
